    ...
    let matches = App::new("ws_mgr").about("Manage websession users/passwords")
        ...
        .subcommand(SubCommand::with_name(ADDUSER)
            .about("Add a new user").arg(Arg::with_name(ID)
                .help("Identifier for new user").required(true)
                .takes_value(true).index(1)))
        .subcommand(SubCommand::with_name(DELUSER)
            .about("Delete a user").arg(Arg::with_name(ID)
                .help("Identifier of user to delete").required(true)
                .takes_value(true).index(1)))
        .subcommand(SubCommand::with_name(LOCK)
            .about("Lock a user account").arg(Arg::with_name(ID)
                .help("Identifier of user to lock").required(true)
                .takes_value(true).index(1)))
        .subcommand(SubCommand::with_name(UNLOCK)
            .about("Unlock a user account").arg(Arg::with_name(ID)
                .help("Identifier of user to unlock").required(true)
                .takes_value(true).index(1)))
        .subcommand(SubCommand::with_name(PASSWD)
            .about("Reset the password for an account (also unlocks it)").
            arg(Arg::with_name(ID).help("Identifier of user to reset")
                .required(true).takes_value(true).index(1)))
        .subcommand(SubCommand::with_name(INFO)
            .about("Get user information").arg(Arg::with_name(ID)
                .help("Identifier of user (optional)").required(false).
                takes_value(true).index(1)))
        .settings(&[
